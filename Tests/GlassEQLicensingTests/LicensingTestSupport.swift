...
final class InMemoryCredentialStore: LicenseCredentialStore {
    private struct State: Sendable {
        ...
        var activationLoadFailure: LicenseCredentialStoreError?
    ...
    func failNextActivationSave(with error: LicenseCredentialStoreError) {
        state.withLock { $0.nextActivationSaveFailures.append(error) }
    }
    ...
    var activationLoadFailure: LicenseCredentialStoreError? {
        get { state.withLock { $0.activationLoadFailure } }
        set { state.withLock { $0.activationLoadFailure = newValue } }
    }
    ...
    func loadActivationState() throws(LicenseCredentialStoreError) -> ActivationState? {
        let result = state.withLock { ($0.loadFailure, $0.activationLoadFailure, $0.activation) }
        if let failure = result.0 { throw failure }
        if let failure = result.1 { throw failure }
        return result.2
    }
    ...
    func saveActivationState(_ activation: ActivationState) throws(LicenseCredentialStoreError) {
        let failure = state.withLock { state -> LicenseCredentialStoreError? in
            ...
            if !state.nextActivationSaveFailures.isEmpty {
                return state.nextActivationSaveFailures.removeFirst()
            }
            guard state.saveFailure == nil else { return state.saveFailure }
    ...
    func clearActivationState() throws(LicenseCredentialStoreError) {
        let failure = state.withLock { state -> LicenseCredentialStoreError? in
            ...
            state.activationLoadFailure = nil
            return nil
        }
        if let failure { throw failure }
    }
