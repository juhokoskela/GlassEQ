...
struct SeparateClockStallExperimentTests {
    ...
    func sourceStopsWhileRingIsFull(outputRate: Double) throws {
        ...
        withKnownIssue("A full ring retains old audio after source silence; see the September 20 stall experiment.") {
            #expect(recovery.dropFirst().allSatisfy { $0.isSilent })
        }
        #expect(recovery.suffix(16).allSatisfy { $0.isSilent })
        #expect(experiment.runtime.snapshotMetrics().adaptivePlaybackRenderFailures == 0)
...
// a device or process tap. One synthetic tick is 960 capture frames / 20 ms. Withholding a
...
private final class StallExperiment {
    ...
    let primeFrames = 3_072
    let outputRate: Double
    let outputFrames: Int
    private var sourceFrame = 0

    init(outputRate: Double, preampDB: Double = 0) throws {
        self.outputRate = outputRate
        self.outputFrames = Int(outputRate / 50)
        runtime = SeparateClockAudioBackend.AudioRuntime(
            ...
            scratchFrames: 1_024, captureCallbackFrames: 960, playbackPrimeFrames: primeFrames
        )
    ...
    func run(callbacks: Int, capture: Bool = true, playback: Bool = true, silentSource: Bool = false) -> [Observation] {
        ...
        for _ in 0..<callbacks {
            if capture {
                var input = [Float](repeating: 0, count: 960 * 2)
                if !silentSource {
                    for frame in 0..<960 {
            ...
            if playback {
                ...
                let age = midpoint > 0.05
                    ? (Double(sourceFrame + 960) - (midpoint - 0.1) * 1_000_000) / 48
                    : nil
