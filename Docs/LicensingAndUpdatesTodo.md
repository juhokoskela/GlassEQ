...
- [x] **L1: Complete the billing queue consumer.** Checkout, Invoice, Subscription, Refund, and Dispute events are implemented in the server slices below. The worker validates bounded EventBridge envelopes and the pinned source/environment/API version, hydrates current Stripe objects outside transactions, and commits normalized transitions before acknowledging SQS. Order revisions, durable cancellation intent, and adjustment records handle retries and prevent stale events from removing restrictions. Live queue delivery and redrive remain V2.
...
- [ ] **L3: Complete subscription and terminal-state reconciliation.** Paid renewals, payment recovery, cancellation, refunds, disputes, and eligible dispute restoration are implemented. Manual revocation and perpetual offline rights are preserved. Remaining work is the planned daily reconciliation and billing-specific retention for processed events, abandoned orders, and expired delivery material. Existing cleanup covers activation/recovery data, not the full billing lifecycle.
...
The merged monthly slice, [GlassEQServer PR #15](https://github.com/juhokoskela/GlassEQServer/pull/15), adds monthly fulfillment and event-driven subscription reconciliation. Initial paid purchases issue one monthly key and projection. Renewals advance only from paid invoice lines; failed renewal does not grant the next unpaid period. Customer cancellation removes the fourteen-day recovery window, removing a pending cancellation restores it when eligible, and renewal events cannot restore refunded, charged-back, or revoked licenses. An order revision makes concurrent stale Stripe reads retry.

The terminal billing slice, [GlassEQServer PR #16](https://github.com/juhokoskela/GlassEQServer/pull/16), adds full-refund and formal-dispute restrictions, durable monthly cancellation, and eligible dispute restoration. Separate adjustment records preserve other restrictions. Canceled subscriptions retain only eligible paid access after restoration; recurring billing is not restarted. A dispute that blocks initial fulfillment can later release the withheld license and delivery record. Daily reconciliation and billing retention remain L3.

Local verification uses PostgreSQL 18 with all seven migration Up sections applied. The race-enabled server suite, vet, build, vulnerability check, parser fuzzing, and a transactional Down/Up check of migration 00007 passed. New coverage includes renewal refunds, cancellation response loss, rollback after cancellation, stale renewal snapshots, early terminal events, expired restoration, multiple restrictions, and Stripe request parameters through a fake HTTP transport.

These checks establish local protocol and database behavior. Live Stripe/KMS/SQS/SES, Link, and packaged-app acceptance remain unverified. Daily reconciliation, retention, email delivery, update authorization, and production enablement remain open.
