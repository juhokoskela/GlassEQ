...
No licensing or updater PR is currently open in either repository. GlassEQ's open PR #33 concerns an experimental CI runner. The latest published app release is `beta-0.9.3`; [Distribution.md](Distribution.md) describes the beta as ad hoc signed and not notarized.
...
- [ ] **L1: Implement the billing queue consumer.** Validate bounded EventBridge envelopes, source/environment/API version, and the accepted event set. Hydrate current Stripe objects outside transactions; commit idempotent normalized transitions before acknowledging SQS. Handle duplicates, concurrency, reordering, unowned objects, retries, and dead-letter outcomes according to server `Docs/Billing.md`.
- [ ] **L2: Implement paid-order fulfillment.** Validate the purchased item, payment, order association, terms consent, and Checkout email. Atomically create one license/key, monthly projection when applicable, and delivery record; mark the order fulfilled. Add bounded retries for paid orders that have not been fulfilled.
- [ ] **L3: Implement subscription and terminal-state reconciliation.** Cover paid renewals, payment recovery, cancellation, refunds, disputes, and eligible restoration. Preserve manual revocation and perpetual offline rights. Add the planned daily reconciliation and billing-specific retention for processed events, abandoned orders, and expired delivery material. Existing cleanup covers activation/recovery data, not the full billing lifecycle.
...
The `GlassEQServer` worktree beside this app worktree, on `feat/perpetual-purchase-fulfillment`, now implements the perpetual subset of L1-L2: EventBridge/SQS ingestion, current Stripe Checkout hydration, duplicate/concurrent event handling, and atomic license/key/delivery-outbox creation. The worker is opt-in; unsupported billing lifecycles are not acknowledged. L1-L2 remain open for the complete event set and monthly fulfillment.

Local verification uses PostgreSQL 18 with all five current migration Up sections applied. The race-enabled server suite, vet, build, vulnerability check, and a ten-second parser fuzz run passed. Database coverage includes rollback/retry, duplicate events, concurrent events for one order, delayed payment success, metadata/payment-reference rejection, and activation using the encrypted delivery key. This is local protocol and database evidence, not a live Stripe/KMS/SQS/SES or packaged-app acceptance result. Email dispatch and production enablement remain pending.
