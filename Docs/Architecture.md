...
Physical-first startup uses `mutedWhenTapped`, so creating an unattached tap cannot silence an already-playing client before GlassEQ reads it. Complete-composition rebuilds use `muted` taps instead, keeping the new route silent from tap creation until the replacement aggregate IOProc starts reading.
