...
The release script now builds a disk image for every channel with the app, an Applications link, the license, the trademark policy, the source notice, and the Corresponding Source archive. Production builds sign, notarize, and staple the disk image after the app, embed the entitlement public keys from a keys file, and fail without them. A release-evidence file records the notarization submissions, signing identity, toolchain, binary UUIDs, and artifact checksums. When GlassEQ runs from the mounted disk image, a translocated copy, or Downloads, the menu bar popover asks to move it to Applications.
