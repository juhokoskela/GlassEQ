...
GlassEQ therefore retains the complete-composition ordering when rebuilding an already-running combined backend. That path publishes and validates high-quality drift compensation, and the requested explicit-route control did not show client disruption. Physical-first ordering remains the default cold-start path. External-client detection gates promotion only after a failed physical-first start has entered compatibility mode.
...
## Same-device normal-rate transitions need fresh tap state

A packaged build on the built-in speakers exposed a different transition failure. Changing the same physical device from 96 to 44.1 kHz produced a loud recurring pop. The affected 44.1 kHz runtime reported a 512-frame physical buffer and 64-frame aggregate callbacks, 194 callback-start misses with no render overruns, 194 paired input/output discontinuities, and a last paired sample-time correction of 448 frames. Changing volume affected the popping only after a long delay or not at all, which is consistent with stale captured audio rather than a slow DSP callback.

The reverse and adjacent transitions showed the same lifecycle sensitivity. A 44.1-to-96 kHz change popped before compatibility mode took over. A change to 88.2 kHz entered compatibility mode immediately with extreme jitter. A 44.1-to-48 kHz change clicked, entered compatibility mode, and produced silence. In each bad compatibility state, **Retry audio engine** restored clean playback. Returning from the failed 96 kHz state to 44.1 kHz also cleared the symptom.

The unified Core Audio log for the 44.1-to-48 kHz case showed several GlassEQ aggregate IO contexts being created and destroyed during the automatic handoff. One replacement context then received paired timeline re-anchors about every 330 ms, each correcting 15,840 sample frames. Core Audio also reported an Apple Music safety violation of 835 frames, or 18.04 ms, during the churn. The explicit retry created a fresh aggregate and playback recovered. An experimental build that matched the private aggregate to the physical device's 512-frame callback did not prevent the failure and increased the aggregate churn, so that experiment was removed.

Code inspection then found that the automatic combined rebuild reused process taps when the output UID, native stream, and mute policy matched; nominal sample rate was not part of the tap identity. The compatibility backend similarly retained its capture runtime across normal-rate changes and refreshed it only when leaving a route at 24 kHz or below. A second packaged candidate made nominal rate part of combined-tap reuse and recreated compatibility capture whenever a normal-rate output no longer matched the tap runtime.

The hardware run rejected fresh tap identity as a complete fix. The 44.1-to-48 kHz transition remained severely jittery, and the following 88.2 and 96 kHz transitions produced no output. The driver log showed the old combined aggregate still alive while the physical clock changed. HAL stopped and restarted that old IO context several times before GlassEQ's settled rebuild destroyed it. At 88.2 kHz, Apple Music recorded a 3,203-frame, 36.497 ms safety violation during this interval. The replacement aggregate could therefore inherit an already disturbed client or tap state even though GlassEQ later created new process taps.

The next candidate releases the old aggregate and taps as soon as the nominal-rate or stream-format notification arrives, restoring direct playback while the device settles for 650 ms. Only then does it create new process taps. It reads each tap's published format before creating an aggregate, requires both rates to match the physical output, and recreates them on a bounded retry if they are still stale. The compatibility backend remains the failure fallback and the required path for unsettled low-rate Bluetooth routes; it is not the mechanism used to bridge normal-rate transitions.

The packaged candidate passed the same hardware sequence that had failed before: 44.1 to 48 to 88.2 to 96 and back to 44.1 kHz. The severe 48 kHz jitter and silent 88.2 and 96 kHz states did not return. This confirms the tested built-in output and macOS build. Other drivers still need their own transition soak tests.
